...
export default function SeatMap({ sessions, onSelectSeat, selectedSeatId, pendingUnbindSeatId, onSeatUnbound }) {
  ...
  useEffect(() => {
    if (!pendingUnbindSeatId) return;
    unbindSessionFromSeat(pendingUnbindSeatId);
    onSeatUnbound?.();
  }, [pendingUnbindSeatId, onSeatUnbound]);
